...
const FollowersPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const count = followInfoData?.result?.followers;
  ...
  const handleBackClick = () => {
    // Prefer navigating back to where the user came from; fallback to the target user's profile or own profile
    // React Router v6 stores an idx in history state that we can use to detect if we can go back
      ...
      <div className="flex items-center mb-6">
        ...
        <div className="flex flex-col">
          <h1 className="text-xl font-bold">
            {targetName ? `${targetName}'s Followers` : "Followers"}
          </h1>
