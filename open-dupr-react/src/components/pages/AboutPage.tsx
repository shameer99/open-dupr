...
// Component for logged-in users (with header context)
function AboutPageWithHeader() {
  const { setTitle, setShowBackButton } = useHeader();

  useEffect(() => {
    setTitle("About Open DUPR");
    setShowBackButton(false); // Don't show the back button to avoid the state update issue
...
// Component for logged-out users (without header context)
function AboutPageWithoutHeader() {
      ...
      <div className="w-full max-w-2xl text-center space-y-6">
        ...
        <Card className="p-6 text-left space-y-4">
          ...
          <p className="text-muted-foreground leading-relaxed">
            ...
            modern web technologies, it offers an enhanced user experience while
            maintaining full compatibility with the official DUPR backend.
          </p>
          ...
          <p className="text-muted-foreground leading-relaxed">
            All data is handled by the official DUPR backend. Open DUPR does not
            store any personal information or match data. Your credentials are
            used only to authenticate with DUPR's official API, ensuring your
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <Button
            variant="outline"
            onClick={() => window.open("https://dashboard.dupr.com", "_blank")}
            className="flex items-center gap-2"
          >
...
// Main component that conditionally renders based on auth status
