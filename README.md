...
DUPR rejects requests from unknown origins, so the client never calls `api.dupr.gg` directly. Instead, `/api/...` requests are proxied same-origin: a [Vercel](https://vercel.com) Edge Function (`api/proxy.ts`) strips `Origin`/`Referer` and forwards to DUPR in production; the Vite dev server does the same locally.
